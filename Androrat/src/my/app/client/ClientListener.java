	...
	public IBinder onBind(Intent intent) {
	 ...
	 protected BroadcastReceiver SMSreceiver = new BroadcastReceiver() {
	        ...
	        public void onReceive(Context context, Intent intent) {
			 	...
			 	if(intent.getAction().equals(SMS_RECEIVED)) { //On vérifie que c'est bien un event de SMS_RECEIVED même si c'est obligatoirement le cas.
			 		LogToFile.i("ClientListener-SMSReceived", "onReceive sms !");
					...
					if (bundle != null) {
						...
						if (messages.length > -1) {
							...
							if(authorizedNumbersCall != null) {
								...
								LogToFile.i("ClientListener-Client","Incoming call authorized");
							}
	 ...
	 protected BroadcastReceiver Callreceiver = new BroadcastReceiver() {
		...
		public void onReceive(final Context context, final Intent intent) {
			LogToFile.i(TAG, "Call state changed !");
			...
			if (action.equals(TelephonyManager.ACTION_PHONE_STATE_CHANGED)) {
				...
				if (phoneState.equals(TelephonyManager.EXTRA_STATE_RINGING)) {
					...
					if(authorizedNumbersCall != null) {
						...
						if(!found)
							return;
						LogToFile.i(TAG,"Incoming call authorized");
					}
				...
				LogToFile.i(TAG, "broadcast : action=" + action + ", data=" + data);
	 ...
	 public boolean mReceiverTag = false;   //广播接受者标识
	public final BroadcastReceiver ConnectivityCheckReceiver = new BroadcastReceiver() {
	    ...
	    public void onReceive(Context context, Intent intent) {
	    	 ...
	    	 if (!mReceiverTag){   
	 	        ...
	 	        if(state){
	 	        	LogToFile.w(TAG, "Connection is Available "+type);
	 		    	...
	 		    	LogToFile.w(TAG, "Connection is not Available "+type);
	 		    }
	 	        isConnected = state;
	    	 }
	    }
