...
//			    buffer = receiver.read(buffer);
				...
				try {
					...
					connected = false;
					server.getGui().logErrTxt(" Demux: "+e.getMessage());
					/*
					connected = false;
					try {
						clientSocket.close();
				...
				connected = false;
				server.getGui().logErrTxt("InputStream:" + e.getMessage());
				System.out.println("InputStream:" + e.getMessage() + "+e.getCause(): " +e.getCause());
				try {
